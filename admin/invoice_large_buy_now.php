...
include 'include/init.php';
    ...
    if (isset($_GET['id'])) {
    	...
    	$select = $db->query("SELECT * FROM buy_now_order WHERE buy_now_order_id = :id");
    	...
    	$custumer = $order_row->full_name;
        $subtotal = $order_row->product_quantity * $order_row->product_price;
...
$pdf->Cell(112, 5, 'Invoice: #' . $order_row->buy_now_order_id, 0, 1, 'C');
...
$select_product = $db->query("SELECT product_name FROM products WHERE product_id = :p_id");
$select_product = $db->bind(":p_id", $order_row->product_id);

$product_result = $select_product = $db->single();


$p_price = $order_row->product_price;
$formated_price = number_format($p_price, 2, ",", ".");

$p_total = $p_price * $order_row->product_quantity;
$formated_p_total = number_format($p_total, 2, ",", ".");

$pdf->Cell(100, 8, $product_result->product_name, 1, 0, 'C');
$pdf->Cell(20, 8, $order_row->product_quantity, 1, 0, 'C');
...
$pdf->Cell(20, 8, '', 0, 0, 'C');
$pdf->Cell(30, 8, '', 0, 0, 'C');
$pdf->Cell(40, 8, '$ ' . $subtotal_format, 1, 1, 'C');
...
$pdf->Cell(20, 8, '', 0, 0, 'C');
$pdf->Cell(30, 8, '', 0, 0, 'C');
$pdf->Cell(40, 8, '$ ' . $total_format, 1, 1, 'C');
